...
def find_gate(kind="AND", max_try=400):
    """Find a real fly-neuron motif that computes `kind`. Returns a dict or None."""
    gains = {"AND": [0.5, 0.7, 0.4],
             "OR": [0.9, 1.2, 0.7],
             "AND-NOT": [0.5, 0.7, 0.35]}.get(kind, [0.5])
    tried = 0
    for O, A, B, wA, wB, sA, sB in _candidates(kind):
        ...
        if tried > max_try:
            break
        for G in gains:
            out = truth_table(A, B, O, wA, wB, G, sA, sB)
            if classify(out) == kind:
                return {
                    "kind": kind, "A": A, "B": B, "O": O,
                    "wA": wA, "wB": wB, "sA": sA, "sB": sB, "gain": G,
                    "truth": out,
